  ...
  return (
    <>
      <nav className="z-50 flex items-center justify-between w-full py-4 px-4 md:px-16 lg:px-24 xl:px-40 text-sm">
        ...
        <div className="hidden md:flex items-center gap-8 transition duration-500 text-zinc-300">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
        ...
        <div className="flex gap-2">
          <Link
            href="/"
            className="hidden md:block px-7 py-2 bg-primary hover:bg-primary-300 active:scale-95 transition-all rounded-full text-white"
          >
            Get started
          </Link>
          {/* <Link
